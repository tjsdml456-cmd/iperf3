#!/bin/bash
# iperf3 DSCP 시나리오 (DL 전용) — UE1 단일 UDP 세션, 0.5초마다 DSCP 전환
#
# 패턴: 0->32->24->15->0 반복
# UE1만 실행 (UE2/UE3 제거)
...
UE1_FIXED_BITRATE=${UE1_FIXED_BITRATE:-2.5M}
RATE_BASE_M=${RATE_BASE_M:-2.5}
...
LOG_FILE="/tmp/iperf3_dscp_ue1_only_dl.log"
UE1_LOG="/tmp/iperf3_dscp_ue1_only_dl_ue1.log"
...
echo "==========================================" > "$LOG_FILE"
echo "  iperf3 DSCP cycles (DL, UE1 only)" >> "$LOG_FILE"
...
echo "=========================================="
echo "  iperf3 DSCP — UE1 only (${TOTAL_DUR}초)"
echo "=========================================="
echo ""
...
echo "[$(timestamp)] UE1 iperf3 서버 (DL 수신)"
...
kill "$DL0_PID" 2>/dev/null || true
sleep 1
kill -9 "$DL0_PID" 2>/dev/null || true
{ sudo pkill -x iperf3 2>/dev/null || true; } >/dev/null 2>&1
