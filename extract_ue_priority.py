...
def parse_priority_log(log_file: str) -> Dict[int, List[Dict]]:
    ...
    # 로그 라인 패턴: UE{번호} min_combined_prio={값}, prio_weight={값}, ...
    pattern = re.compile(
    ...
    try:
        with open(log_file, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if match:
                    ue_idx = int(match.group(1))
                    entry = {
                        'line': line_num,
                        'min_combined_prio': int(match.group(2)),
                        'prio_weight': float(match.group(3)),
                        'pf_weight': float(match.group(4)),
                        'gbr_weight': float(match.group(5)),
                        'delay_weight': float(match.group(6)),
                        'raw_line': line.strip()
                    }
...
def print_detailed(ue_data: Dict[int, List[Dict]], ue_idx: int = None):
    ...
    for ue in ues_to_print:
        ...
        print(f"\n{'=' * 80}")
        print(f"UE{ue} 상세 정보 (전체 {len(entries)}개)")
        print(f"{'=' * 80}")
        print(f"{'Line':<6} {'min_combined_prio':<18} {'prio_weight':<12} {'pf_weight':<12} {'gbr_weight':<12} {'delay_weight':<12}")
        print("-" * 80)
        
        for entry in entries:  # 전체 출력
            print(f"{entry['line']:<6} {entry['min_combined_prio']:<18} "
...
def export_to_csv(ue_data: Dict[int, List[Dict]], output_file: str):
    """UE별 데이터를 CSV 파일로 저장합니다."""
    import csv
    
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['UE', 'Line', 'min_combined_prio', 'prio_weight', 
                        'pf_weight', 'gbr_weight', 'delay_weight'])
        
        for ue_idx in sorted(ue_data.keys()):
            for entry in ue_data[ue_idx]:
                writer.writerow([
                    ue_idx,
                    entry['line'],
