...
# --- Fixed schedule replay (PCF / DSCP) ---
# PCF CSV: rel_time_s,five_qi  |  DSCP CSV: rel_time_s,dscp
QOS_USE_SCHEDULE=0
QOS_SCHEDULE_MODE=""
QOS_SCHEDULE_REL=()
QOS_SCHEDULE_5QI=()
...
load_qos_schedule_file() {
    ...
    QOS_SCHEDULE_N=$n
    QOS_USE_SCHEDULE=1
    QOS_SCHEDULE_MODE=5qi
    QOS_IDX_SEQ_GENERATED=1
    TRANSITIONS=$((n - 1))
}

load_dscp_schedule_file() {
    local f=$1 line t d n=0
    if [ ! -f "$f" ]; then
        echo "ERROR: DSCP schedule file not found: $f" >&2
        exit 1
    fi
    QOS_SCHEDULE_REL=()
    QOS_SCHEDULE_DSCP=()
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        [ -z "$line" ] && continue
        case "$line" in
            rel_time_s,*|time_s,*|t,*|rel_time,*dscp*) continue ;;
        esac
        t="${line%%,*}"
        d="${line##*,}"
        if ! awk -v t="$t" -v d="$d" 'BEGIN{exit !(t+0==t && d+0==d && d>=0)}'; then
            echo "ERROR: invalid DSCP schedule line: $line" >&2
            exit 1
        fi
        QOS_SCHEDULE_REL[$n]=$t
        QOS_SCHEDULE_DSCP[$n]=$d
        n=$((n + 1))
    done <"$f"
    if [ "$n" -lt 1 ]; then
        echo "ERROR: empty DSCP schedule: $f" >&2
        exit 1
    fi
    QOS_SCHEDULE_N=$n
    QOS_USE_SCHEDULE=1
...
rate_for_dscp() {
    case "$1" in
        44) echo "${QOS_RATE_GBR:-7M}" ;;
        15) echo "${QOS_RATE_DC_GBR:-4M}" ;;
        0)  echo "${QOS_RATE_INITIAL:-${QOS_RATE_NON_GBR:-0.5M}}" ;;
        *)  echo "${QOS_RATE_NON_GBR:-0.5M}" ;;
    esac
}
...
schedule_five_qi_at() {
    if [ "$QOS_SCHEDULE_MODE" = "dscp" ]; then
        dscp_to_five_qi "${QOS_SCHEDULE_DSCP[$1]}"
    else
...
schedule_dscp_at() {
    if [ "$QOS_SCHEDULE_MODE" = "dscp" ]; then
        ...
        five_qi_to_dscp "${QOS_SCHEDULE_5QI[$1]}"
    fi
}
