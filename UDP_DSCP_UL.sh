#!/bin/bash
# iperf3 DSCP 시나리오 (UL 전용) — UE1 UDP cycle only
# (UE2/UE3 TCP background는 주석 처리 — 1UE UL QRT/용량 실험용)
#
...
DSCP_SCHEDULE_FILE=${DSCP_SCHEDULE_FILE:-"$SCRIPT_DIR/qos_schedules/qos_schedule_dscp.csv"}
...
echo "==========================================" >"$LOG_FILE"
echo "  iperf3 DSCP cycles (UL, UE1 only)" >>"$LOG_FILE"
...
echo "=========================================="
echo "  iperf3 DSCP UL — UE1 only (${TOTAL_DUR}초)"
echo "=========================================="
echo ""
...
echo "  UE2/UE3 BG: disabled (UE1 only)"
echo ""
echo "  [타이밍 치트시트 — 1사이클]"
print_phase_timing_cheatsheet
echo ""
log_event "시작 USE_RATE_CHANGE=$USE_RATE_CHANGE (UE1 only)"
...
echo "[$(timestamp)] host/DN iperf3 server 시작 (UL 수신, UE1 only)"
...
echo "=========================================="
echo "[$(timestamp)] 트래픽 시작 (${TOTAL_DUR}초) — UL UE1 only"
...
log_event "PIDs UE1=$UE1_PID (UE2/UE3 disabled)"
log_event "트래픽 기준 EPOCH_NS: $TRAFFIC_START_EPOCH_NS"
log_event "throughput 추출: MAC-THP-UL / UE0(=UE1)"
...
if [ "$EARLY_EXIT" -eq 1 ] || [ "$UE1_RC" -ne 0 ]; then
    echo "[$(timestamp)] ERROR: iperf3 조기 종료 (UE1 rc=${UE1_RC})"
...
# kill "$UE2_PID" "$UE3_PID" 2>/dev/null || true
sleep 1
{ sudo pkill -x iperf3 2>/dev/null || true; } >/dev/null 2>&1
